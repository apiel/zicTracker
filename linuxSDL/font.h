...
void draw_char(SDL_Surface* surface, unsigned char symbol, int x, int y, unsigned short color)
{
    ...
    const unsigned char* ptr = embedded_font + symbol * h;

    for (int i = 0, ys = 0; i < w; i++, ptr++, ys += 1) {
        for (int col = h - w, xs = x - col; col < h; col++, xs -= 1) {
            if ((*ptr & 1 << col) && y + ys < surface->h && xs < surface->w) {
                SDL_Rect r = { xs * scale, ys * scale, scale, scale };
